...
PatmosTargetLowering::PatmosTargetLowering(const PatmosTargetMachine &tm,
  ...
  // Where divrem? It caused compiler issues being compilable in the end, and
  // wasn't really used anymore. TODO: Maybe find a way to renable them again
...
const MCExpr *PatmosTargetLowering::LowerCustomJumpTableEntry(
    ...
    unsigned uid, MCContext &OutContext) const {
...
SDValue PatmosTargetLowering::LowerLOAD(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue newLoad = DAG.getLoad(
      ISD::UNINDEXED, ISD::ZEXTLOAD, MVT::i32, Op, load->getChain(),
      load->getBasePtr(), load->getOffset(), MVT::i8, load->getMemOperand());
...
SDValue PatmosTargetLowering::LowerSTORE(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getTruncStore(store->getChain(), Op, newVal, store->getBasePtr(),
                           MVT::i1, store->getMemOperand());
}
...
SDValue PatmosTargetLowering::LowerMUL_LOHI(SDValue Op,
  ...
  MultOpc =
      (Op.getOpcode() == ISD::UMUL_LOHI) ? PatmosISD::MULU : PatmosISD::MUL;

  SDValue Mul =
      DAG.getNode(MultOpc, dl, MVT::Glue, Op.getOperand(0), Op.getOperand(1));

  SDValue InChain = DAG.getEntryNode();
  SDValue InGlue = Mul;

  SDValue CopyFromLo = DAG.getCopyFromReg(InChain, dl, Patmos::SL, Ty, InGlue);
  DAG.ReplaceAllUsesOfValueWith(Op.getValue(0), CopyFromLo);
  InChain = CopyFromLo.getValue(1);
  InGlue = CopyFromLo.getValue(2);

  SDValue CopyFromHi = DAG.getCopyFromReg(InChain, dl, Patmos::SH, Ty, InGlue);
...
SDValue PatmosTargetLowering::LowerRETURNADDR(SDValue Op,
  ...
  if (Depth) {
    report_fatal_error(
        "Return address can only be determined for the current frame in " +
        DAG.getMachineFunction().getName());
  }

  // TODO we only return the offset here .. how can we make this both a base and
  // offset??
  SDValue RetAddr = DAG.getCopyFromReg(DAG.getEntryNode(), dl, Patmos::SRO, VT);
  return RetAddr;
}
...
SDValue PatmosTargetLowering::LowerFRAMEADDR(SDValue Op,
  ...
  if (Depth) {
    report_fatal_error(
        "Frame address can only be determined for current frame in " +
        DAG.getMachineFunction().getName());
  }

  SDValue FrameAddr =
      DAG.getCopyFromReg(DAG.getEntryNode(), DL, Patmos::RFP, VT);
  return FrameAddr;
}
    ...
    if (G->getGlobal()->getName() == "llvm.loop.bound") {
      ...
      SDValue Ops[] = {
          ...
          DAG.getTargetConstant(
              cast<ConstantSDNode>(OutVals[0].getNode())->getZExtValue(), dl,
              MVT::i32),
          DAG.getTargetConstant(
              cast<ConstantSDNode>(OutVals[1].getNode())->getZExtValue(), dl,
              MVT::i32)};
...
// FIXME: varargs
SDValue PatmosTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
  ...
  PatmosMachineFunctionInfo &PMFI =
      *DAG.getMachineFunction().getInfo<PatmosMachineFunctionInfo>();
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i32:
        unsigned VReg = RegInfo.createVirtualRegister(&Patmos::RRegsRegClass);
  ...
  if (isVarArg) {
    ...
    PMFI.setVarArgsFI(MFI.CreateFixedObject(4, CCInfo.getStackSize(), true));
  }
...
SDValue
PatmosTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                  bool isVarArg,
...
///
SDValue PatmosTargetLowering::LowerCallResult(
  ...
  for (unsigned i = 0; i != RVLocs.size(); ++i) {
    assert((RVLocs[i].getLocReg() == Patmos::R1 ||
            RVLocs[i].getLocReg() == Patmos::R2 ||
            RVLocs[i].getLocReg() == Patmos::R3 ||
            RVLocs[i].getLocReg() == Patmos::R4) &&
    ...
    SDValue val =
        DAG.getCopyFromReg(Chain, dl, RVLocs[i].getLocReg(), MVT::i32, InFlag);
...
PatmosTargetLowering::ConstraintType
PatmosTargetLowering::getConstraintType(StringRef Constraint) const {
...
PatmosTargetLowering::getRegForInlineAsmConstraint(
  ...
  // This ensures a sensible error message is printed if anyone accidentally
  // does it.
  if (Constraint.size() > 2 && Constraint[0] == '{' && Constraint[1] == '$') {
    report_fatal_error("Inline assembly clobbers cannot have '$' preceding "
                       "clobbered registers");
  }
  return TargetLowering::getRegForInlineAsmConstraint(TRI, Constraint, VT);
}

SDValue PatmosTargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  ...
  SDLoc dl(Op);
  SDValue VarArgsFI =
      DAG.getFrameIndex(PMFI.getVarArgsFI(), getPointerTy(DAG.getDataLayout()));
...
const char *PatmosTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  default:
    return NULL;
  case PatmosISD::RET_FLAG:
    return "PatmosISD::RET_FLAG";
  case PatmosISD::CALL:
    return "PatmosISD::CALL";
  case PatmosISD::MUL:
    return "PatmosISD::MUL";
  case PatmosISD::MULU:
    return "PatmosISD::MULU";
  case PatmosISD::LOOP_BOUND:
    return "PatmosISD::LOOP_BOUND";
  }
}
