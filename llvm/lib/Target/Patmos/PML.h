...
#define YAML_IS_PTR_SEQUENCE_VECTOR(_type)                                     \
  template <> struct SequenceTraits<std::vector<_type *>> {                    \
...
#define YAML_IS_PTR_SEQUENCE_VECTOR_1(_type)                                   \
  ...
  struct SequenceTraits<std::vector<_type<_member_type> *>> {                  \
    ...
    static _type<_member_type> *&                                              \
    element(IO &io, std::vector<_type<_member_type> *> &seq, size_t index) {   \
      if (index >= seq.size())                                                 \
        seq.resize(index + 1);                                                 \
      return seq[index];                                                       \
    }                                                                          \
...
#define YAML_IS_SEQUENCE_VECTOR(_type)                                         \
  template <> struct SequenceTraits<std::vector<_type>> {                      \
    static size_t size(IO &io, std::vector<_type> &seq) { return seq.size(); } \
    static _type &element(IO &io, std::vector<_type> &seq, size_t index) {     \
      if (index >= seq.size())                                                 \
        seq.resize(index + 1);                                                 \
      return seq[index];                                                       \
...
#define YAML_IS_PTR_SEQUENCE_VECTOR_2(_type)                                   \
  ...
  struct SequenceTraits<std::vector<_type<_member_type1, _member_type2> *>> {  \
    static size_t                                                              \
    size(IO &io, std::vector<_type<_member_type1, _member_type2> *> &seq) {    \
      return seq.size();                                                       \
    }                                                                          \
    static _type<_member_type1, _member_type2> *&                              \
    element(IO &io, std::vector<_type<_member_type1, _member_type2> *> &seq,   \
            size_t index) {                                                    \
      if (index >= seq.size())                                                 \
        seq.resize(index + 1);                                                 \
      return seq[index];                                                       \
...
template <> struct ScalarEnumerationTraits<ReprLevel> {
...
template <> struct ScalarEnumerationTraits<MemMode> {
...
template <> struct ScalarEnumerationTraits<BranchType> {
...
struct MachineInstruction : Instruction {
  ...
  MachineInstruction(uint64_t Index)
      : Instruction(Index), Size(0), Address(-1), BranchType(branch_none),
        BranchDelaySlots(0), StackCacheArg(0), StackCacheFill(0),
        StackCacheSpill(0), MemType(""), Bundled(false) {}
};
template <> struct MappingTraits<MachineInstruction *> {
  static void mapping(IO &io, MachineInstruction *&Ins) {
    if (!Ins)
      Ins = new MachineInstruction(0);
    io.mapRequired("index", Ins->Index);
    io.mapOptional("opcode", Ins->Opcode, "");
    io.mapOptional("callees", Ins->Callees);
    ...
    io.mapOptional("branch-targets", Ins->BranchTargets,
                   std::vector<UnsignedValue>());
...
template <typename NameT, typename InstructionT> struct Block {
  NameT BlockName;
  StringValue MapsTo;
  int64_t Address;
...
struct MappingTraits<Block<NameT, InstructionT> *> {
  static void mapping(IO &io, Block<NameT, InstructionT> *&B) {
    if (!B)
      B = new Block<NameT, InstructionT>(0ULL);
...
struct Argument {
  ...
  Argument(StringRef name, uint64_t index)
      : ArgName(name.str()), Index(index) {}

  void addReg(const StringRef regname) { Registers.push_back(regname.str()); }
};
...
struct Subfunction {
  ...
  void addBlock(uint64_t index) { Blocks.push_back(index); }
};
template <> struct MappingTraits<Subfunction *> {
  static void mapping(IO &io, Subfunction *&S) {
    if (!S)
      report_fatal_error("NULL Subfunction");
...
/// basic functions
template <typename NameT, typename BlockT> struct Function {
  NameT FunctionName;
  ReprLevel Level;
  StringValue MapsTo;
  UnsignedValue Hash;
...
struct MappingTraits<Function<NameT, BlockT> *> {
  static void mapping(IO &io, Function<NameT, BlockT> *&Fn) {
    if (!Fn)
      Fn = new Function<NameT, BlockT>(0ULL);
...
template <> struct ScalarEnumerationTraits<RelationNodeType> {
...
struct RelationNode {
  ...
  // Initialize all fields to avoid uninitialized values in YAML output,
  // which causes flow facts to map to wrong machine blocks (INFEASIBLE ILP).
  // Otherwise Rust based tooling will not work (due to how Rust does with
  // immidiate representation (IR)), does not affect Clang (C++/C).
  RelationNode(UnsignedValue name, RelationNodeType type)
      : NodeName(name), NodeType(type), SrcBlock(""), DstBlock(0) {}
...
template <> struct MappingTraits<RelationNode *> {
  static void mapping(IO &io, RelationNode *&node) {
    if (!node)
      report_fatal_error("NULL RelationNode");
    io.mapRequired("name", node->NodeName);
    io.mapRequired("type", node->NodeType);
    // Only write src-block for src/progress/entry nodes.
    // dst-block for dst/progress/entry nodes (entry needs dst-block for machine
    // block 0). Exit nodes should NOT have dst-block to avoid spurious default
    // values (0).
    if (node->NodeType == rnt_src || node->NodeType == rnt_progress ||
        node->NodeType == rnt_entry)
      io.mapOptional("src-block", node->SrcBlock);
    if (node->NodeType == rnt_dst || node->NodeType == rnt_progress ||
        node->NodeType == rnt_entry)
      io.mapOptional("dst-block", node->DstBlock);
    // NOTE: rnt_exit deliberately excluded from dst-block to prevent INFEASIBLE
    // ILP in platin. Exit nodes represent function exit, not a specific machine
    // block.
...
template <typename NameT> struct RelationScope {
  NameT Function; // XXX: should be a scope, really
  ReprLevel Level;

  RelationScope(NameT f, ReprLevel level) : Function(f), Level(level) {}
};
template <typename NameT> struct MappingTraits<RelationScope<NameT> *> {
  static void mapping(IO &io, RelationScope<NameT> *&scope) {
    if (!scope)
      report_fatal_error("Cant't create RelationScope");
...
/// corrected: initial mapping did not include all path, but tabu list corrected
/// the problem incomplete: no sensible mapping that includes all paths from
/// both graphs was found
enum RelationGraphStatus {
  rg_status_valid,
  rg_status_loop,
  rg_status_corrected,
  rg_status_incomplete
};
template <> struct ScalarEnumerationTraits<RelationGraphStatus> {
...
struct RelationGraph {
  ...
  RelationGraph(RelationScope<StringValue> *src,
                RelationScope<UnsignedValue> *dst)
...
template <> struct MappingTraits<RelationGraph *> {
  static void mapping(IO &io, RelationGraph *&RG) {
    if (!RG)
      RG = new RelationGraph(0, 0);
...
struct ContextEntry {
  ...
  ContextEntry(StringValue loop, uint64_t offset, uint64_t step)
      : Loop(loop), Offset(offset), Step(step) {}
  ContextEntry(StringValue callsite) : Callsite(callsite) {}
  // for YAML I/O
  ContextEntry() {}
};
template <> struct MappingTraits<ContextEntry *> {
...
template <typename NameT> struct Scope {
  NameT Function;
  NameT Loop;
  std::vector<ContextEntry *> Context;

  Scope(const NameT &f) : Function(f) {}
  ~Scope() { DELETE_PTR_VEC(Context); }

  Scope(const Scope<NameT> &Src) : Function(Src.Function), Loop(Src.Loop) {
...
template <typename NameT> struct MappingTraits<Scope<NameT> *> {
  static void mapping(IO &io, Scope<NameT> *&S) {
    if (!S)
      report_fatal_error("Cant't create Scope");
...
struct ProgramPoint {
  ...
  std::vector<ContextEntry *> Context;
  ProgramPoint() {}
  ~ProgramPoint() { DELETE_PTR_VEC(Context); }
  static ProgramPoint *CreateFunction(const StringValue &Function) {
    ProgramPoint *PP = new ProgramPoint();
    PP->Function = Function;
    return PP;
  }
  static ProgramPoint *CreateBlock(const StringValue &Function,
                                   const StringValue &Block) {
    ProgramPoint *PP = CreateFunction(Function);
    PP->Block = Block;
    return PP;
  }
  static ProgramPoint *CreateInstruction(const StringValue &Function,
                                         const StringValue &Block,
                                         const StringValue &Instruction) {
  ...
  // Custom copy constructor and assignment, as the Context members are owned by
  // ProgramPoint
  ProgramPoint(const ProgramPoint &Src) { copyFrom(Src); }
...
template <> struct MappingTraits<ProgramPoint *> {
...
template <> struct MappingTraits<Value> {
...
struct ValueFact {
  ...
  void addValue(int64_t min, int64_t max) { Values.push_back(Value(min, max)); }

  void addValue(StringValue symbol) { Values.push_back(Value(symbol)); }

private:
  ValueFact(const ValueFact &);            // Disable copy constructor
  ValueFact *operator=(const ValueFact &); // Disable assignment
};
template <> struct MappingTraits<ValueFact *> {
  static void mapping(IO &io, ValueFact *&VF) {
    if (!VF)
      VF = new ValueFact(level_machinecode);
...
template <> struct ScalarEnumerationTraits<CmpOp> {
...
template <> struct MappingTraits<Term> {
...
template <typename NameT> struct FlowFact {
  StringValue Origin;
  ReprLevel Level;
...
template <typename NameT> struct MappingTraits<FlowFact<NameT> *> {
  static void mapping(IO &io, FlowFact<NameT> *&FF) {
    if (!FF)
      report_fatal_error("Cant't create FlowFact");
...
template <> struct MappingTraits<Timing *> {
  static void mapping(IO &io, Timing *&T) {
    if (!T)
      T = new Timing(level_machinecode);
...
template <typename FuncT, typename NameT> struct PMLDoc {
  ...
  PMLDoc(const char *FunctionLabel, StringRef TargetTriple)
      : FormatVersion("pml-0.1"), TargetTriple(TargetTriple),
        FunctionLabel(FunctionLabel) {}
  ...
  void addFunction(FuncT *F) { Functions.push_back(F); }
  /// Add a relation graph, which is owned by the document afterwards
  void addRelationGraph(RelationGraph *RG) { RelationGraphs.push_back(RG); }
  /// Add a valuefact, which is owned by the document afterwards
  void addValueFact(ValueFact *VF) { ValueFacts.push_back(VF); }
  /// Add a flowfact, which is owned by the document afterwards
  void addFlowFact(FlowFact<NameT> *FF) { FlowFacts.push_back(FF); }

  bool empty() {
    return Functions.empty() && RelationGraphs.empty() && ValueFacts.empty() &&
  ...
  void mergePML(PMLDoc<FuncT, NameT> &Doc) {
    ...
    if (TargetTriple.empty())
      TargetTriple = Doc.TargetTriple;

    Functions.insert(Functions.end(), Doc.Functions.begin(),
                     Doc.Functions.end());
    Doc.Functions.clear();
    RelationGraphs.insert(RelationGraphs.end(), Doc.RelationGraphs.begin(),
                          Doc.RelationGraphs.end());
    Doc.RelationGraphs.clear();
    ValueFacts.insert(ValueFacts.end(), Doc.ValueFacts.begin(),
                      Doc.ValueFacts.end());
    Doc.ValueFacts.clear();
    FlowFacts.insert(FlowFacts.end(), Doc.FlowFacts.begin(),
                     Doc.FlowFacts.end());
    Doc.FlowFacts.clear();
    Timings.insert(Timings.end(), Doc.Timings.begin(), Doc.Timings.end());
    Doc.Timings.clear();
  }
...
struct MappingTraits<PMLDoc<FuncT, NameT> *> {
  static void mapping(IO &io, PMLDoc<FuncT, NameT> *&doc) {
    if (!doc)
      report_fatal_error("Cant't create PMLDoc");
...
template <typename FuncT, typename NameT> struct PMLDocList {

  std::vector<PMLDoc<FuncT, NameT> *> YDocs;

  ~PMLDocList() { DELETE_PTR_VEC(YDocs); }
